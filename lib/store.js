...
function sendResponList(groupId, key, _dir) {
    ...
    if (position !== null) {
        const responseText = _dir[position].response;
        const mediaUrl = _dir[position].mediaUrl || null; // Cek apakah ada media URL

        // Mengembalikan objek berisi respons teks dan URL media jika ada
...
const expiredCheck = async (alpha, _dir) => {
    const now = Date.now();
    const expiredGroups = _dir.filter(group => now >= group.expired);

    if (expiredGroups.length > 0) {
        for (const group of expiredGroups) {
            console.log(`Sewa expired: ${group.id}`);
            await alpha.sendMessage(group.id, { text: `Masa sewa di grup ini telah habis, bot otomatis keluar!` });
            await alpha.groupLeave(group.id);
        }

        // Update JSON setelah penghapusan
        _dir = _dir.filter(group => now < group.expired);
        await writeFileAsync('./database/sewa.json', JSON.stringify(_dir, null, 2));
    }

    // Jadwalkan ulang cek berdasarkan grup yang akan expired selanjutnya
    const nextExpire = Math.min(..._dir.map(group => group.expired), Infinity) - now;
    if (nextExpire > 0 && nextExpire !== Infinity) {
        setTimeout(() => expiredCheck(alpha, _dir), nextExpire);
    }
};
